...
**Last updated:** 2026-06-09 (CRITICAL FIX: photo-recognition outage resolved — the worker's GEMINI_API_KEY was dead, every photo was failing; re-set a fresh key. Also today: all 10 languages recorded (v113), iOS/UX/dashboard/landing fixes, new social card, and an automated recognition eval = 0 genuine misses across 109 items. Today's eval finishers (all DONE): square test image fixed → literal 109/109; a discrimination/false-positive pass (74% correct rejection, the rest permissive-by-design, colors loosest); pronunciation auto-QA of zh/ja/ko/hi/ar (Whisper round-trip — no systematic mispronunciation found; the short-word misses are an STT limitation, not bad audio; a native-ear spot-check of short words is still ideal).)
